...
Raid2X.prototype.readFile = function(file, callback) {
	...
	reader.onload = function() {
		...
		callback(undefined, self.readBuffer(ui8a));
	};
...
Raid2X.prototype.setHash = function(hash) {
	...
	else if(new String(this.binary) == "[object Uint8Array]") {
...
Raid2X.prototype.getShardList = function(reset) {
	if(reset || !this.checkShard()) {
		this.shardList = new Array(this.attr.sliceCount * 2);
		if(new String(this.binary) == "[object Uint8Array]") {
...
Raid2X.prototype.getShard = function(n, type) {
	...
	if(this.attr.duplicate) {
		var addi = new Uint8Array(new String(n).split("").map(function(c) { return c.charCodeAt(0); }));
		shard = new Uint8Array(this.binary.length + addi.length);
		shard.set(this.binary, 0);
		shard.set(addi, this.binary.length);
	}
		...
		var tmpBinary = new Uint8Array(this.attr.size + this.attr.sliceSize)
		tmpBinary.set(this.binary, 0);
		shard = new Uint8Array(tmpBinary.subarray(n * this.attr.sliceSize, (n + 1) * this.attr.sliceSize));
	}

	if(this.attr.encShard) { shard = this.encrypt(shard); }
	switch(type) {
		case 'base64':
			shard = btoa(String.fromCharCode.apply(null, shard));
			break;
