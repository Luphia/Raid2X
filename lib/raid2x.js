	...
	var option = {
		...
		callback: function(error, resp) {
			if(error) {
				...
				var target.path = self.changePath(target.path);
...
Raid2X.prototype.changePath = function (path) {
	return this.currNode + path.split('/shards/').slice(1).join('/shards/');
};
...
Raid2X.prototype.downloadAll = function(path, callback) {
	...
	var addDownload = function() {
		...
		p = typeof(path) == 'function'? path(h): !!currNode? currNode + h: path + h;
